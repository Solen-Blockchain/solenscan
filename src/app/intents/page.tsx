    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1.5 text-2xl font-semibold text-gray-900">{pending.length}</p>
          <p className="text-xs text-gray-400 mt-0.5">awaiting solvers</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Fulfilled</p>
          <p className="mt-1.5 text-2xl font-semibold text-gray-900">{fulfilled.length}</p>
          <p className="text-xs text-gray-400 mt-0.5">completed</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Total Tips Paid</p>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {pending.map((intent) => (
                      <tr key={intent.id} className="hover:bg-gray-50">
                        <td className="px-5 py-3 font-medium text-gray-900">#{intent.id}</td>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex flex-wrap gap-1">
                            {intent.constraints.map((c, i) => (
                              <span key={i} className="inline-flex items-center rounded-full bg-cyan-50 px-2 py-0.5 text-xs font-medium text-cyan-700 ring-1 ring-cyan-600/20">
                                {c.type}
                              </span>
                            ))}
                          </div>
                ...
                <table className="min-w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {fulfilled.map((intent) => (
                      <tr key={intent.intent_id} className="hover:bg-gray-50">
                        <td className="px-5 py-3">
